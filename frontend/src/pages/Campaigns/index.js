...
const Campaigns = () => {
  ...
  useEffect(() => {
    const storedWhatsappId = localStorage.getItem("selectedWhatsappId");
    if (storedWhatsappId) {
      setWhatsappId(parseInt(storedWhatsappId));
    }
  }, []);

  useEffect(() => {
    if (whatsappId) {
      localStorage.setItem("selectedWhatsappId", whatsappId.toString());
    }
  }, [whatsappId]);
  ...
  defaultWhatsappId={
    selectedCampaign?.whatsappId || whatsappId || null
  }
/>
